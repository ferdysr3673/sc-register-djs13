  ...
  async execute(message, args, client) {
    ...
    try {
      if (!message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) {
        const embed = new MessageEmbed().setColor('RANDOM').setTitle('✅ • Success').setDescription(`> ${message.author} *Accept, Kamu sudah terverifikasi di server **${message.guild.name}***`).setFooter(message.author.username, message.author.displayAvatarURL()).setTimestamp();
